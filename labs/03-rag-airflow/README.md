...
3. ให้คุณสร้างไฟล์ข้อมูลชื่อ `new_doc.txt` (พิมพ์อะไรก็ได้ที่เกี่ยวกับข้อมูลสวัสดิการของพนักงาน) แล้วนำไปวางไว้ในโฟลเดอร์ `./data/new_doc.txt` ของเครื่องคอมพิวเตอร์คุณ
4. สังเกตบนหน้าจอ Airflow UI: Sensor จะตรวจพบไฟล์โดยอัตโนมัติ, ระบบจะดึงข้อมูลไปแบ่งหน้า (Chunk), แปลงเวกเตอร์ลง ChromaDB และสรุปผลผ่าน Gemini (`summarize_new_data`) จนท่อส่งแสดงผลสำเร็จเป็นสีเขียว
